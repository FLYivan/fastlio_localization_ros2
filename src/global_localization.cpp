...
private:
    ...
    void global_localization(const Eigen::Matrix4d& pose_estimation)
    {
        ...
        auto coarse = registration_at_scale(scan_ds5, map_ds5, pose_estimation, 5.0);

        auto scan_ds1 = voxel_down_sample(scan_copy, scan_voxel_size_ * 1);
        auto map_ds1 = voxel_down_sample(map_in_fov, map_voxel_size_ * 1);

        auto fine = registration_at_scale(scan_ds1, map_ds1, coarse.transformation, 1.0);

        // RCLCPP_INFO(this->get_logger(), "ICP coarse fitness=%.4f fine fitness=%.4f",
        //             coarse.fitness, fine.fitness);

        if (fine.fitness < localization_threshold_) {
            RCLCPP_WARN(this->get_logger(), "ICP fitness %.4f < threshold %.4f, rejected",
                        fine.fitness, localization_threshold_);
            return;
        }
    ...
    {
        Eigen::Matrix4d initial_pose = pose_to_mat(msg->pose.pose);

        {
            std::lock_guard<std::mutex> lock(mutex_);
            T_map_to_odom_ = initial_pose;
            initialized_ = true;
            RCLCPP_INFO(this->get_logger(), "Initial pose received");
        }

        if (scan_received_) {
            do_localization(initial_pose);
        }
    }
